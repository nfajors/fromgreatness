...
export default function AssessmentsPage() {
  ...
  const getStatus = (type: string): AssessmentStatus => {
    ...
    const a = dbAssessments.find(x => x.type === type);
    if (!a) return type === 'personality' ? 'not_started' : 'locked';
  ...
  const handleStart = async (id: string) => {
    if (selectedStudentId) {
      await getOrCreate.mutateAsync({
        ...
        type: id as 'personality' | 'achievement' | 'cultural_identity',
      });
    }
  ...
  const handlePersonalityComplete = async () => {
    const personalityAssessment = dbAssessments?.find(a => a.type === 'personality');
    if (personalityAssessment && selectedStudentId) {
      // Calculate top traits from answers
      const traitScores = Object.values(answers.personality || {}).reduce((sum: number, v) => sum + (typeof v === 'number' ? v : 0), 0);
      const avgScore = Object.keys(answers.personality || {}).length > 0 ? traitScores / Object.keys(answers.personality || {}).length : 50;
      await submitAssessment.mutateAsync({
        id: personalityAssessment.id,
        responses: answers.personality,
        score: Math.round(avgScore),
        traits: avgScore > 70 ? ['Curious', 'Creative', 'Determined'] : avgScore > 40 ? ['Thoughtful', 'Balanced'] : ['Growing', 'Learning'],
      });
    }
    setView('overview');
  };
  ...
  const handleAchievementComplete = async () => {
    const assessment = dbAssessments?.find(a => a.type === 'achievement');
  ...
  const handleCulturalComplete = async () => {
    const assessment = dbAssessments?.find(a => a.type === 'cultural_identity');
    if (assessment && selectedStudentId) {
      await submitAssessment.mutateAsync({
        id: assessment.id,
